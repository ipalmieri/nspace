...
void neuron::normalizeWeights()
{
   ...
   for (edgeMap::const_iterator it = _node->inputEdgesBegin(); it != _node->inputEdgesEnd(); ++it)
   {
...
vector<synapse *> neuron::getOutputSynapses() const
{
   ...
   for (edgeMap::const_iterator it = _node->outputEdgesBegin(); it != _node->outputEdgesEnd(); ++it)
      ret.push_back((synapse *)(it->second)->getProperty());
