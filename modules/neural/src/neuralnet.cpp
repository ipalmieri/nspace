...
neuralNet::~neuralNet()
{
  ...
  for (vector<afunction*>::iterator it = _functions.begin();
...
bool neuralNet::updateLoop(const unsigned& maxiter, const Real& mindelta)
{
  ...
  do {
    ...
    for (nodeMap::const_iterator it = _graph->nodeMapBegin();
      ...
      if (abs(oldval - nr->getOutput()) >= mindelta) {
        convStatus = false;
      }
    }
    ...
    for (edgeMap::const_iterator it = _graph->edgeMapBegin();
      ...
      if (abs(oldval - sn->getOutput()) >= mindelta) {
        convStatus = false;
      }
    }
...
void neuralNet::setInput(const vectorN<nsignal>& insignal)
{
  assert(insignal.size() == _ivars.size());
