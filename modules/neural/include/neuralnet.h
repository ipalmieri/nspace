...
class neuralNet
{
 public:
  ...
  inline unsigned int neuronCount() const
  {
    return _graph->nodeCount();
  }
  inline unsigned int synapseCount() const
  {
  ...
  void updateNeurons();
  void updateSynapses();
  bool updateLoop(const unsigned& maxiter,
                  const tools::Real& mindelta=_MIN_DELTA);

  virtual void setInput(const tools::vectorN<nsignal>& insignal);
  inline unsigned int inputSize() const
  {
    return _ivars.size();
  }
  inline ineuron* getInputNeuron(const unsigned int& index)
  {
    assert(index < _ivars.size());
    return _ivars[index];
  }

  tools::vectorN<nsignal> getOutput() const;
  inline unsigned int outputSize() const
  {
    return _ovars.size();
  }
  inline neuron* getOutputNeuron(const unsigned int& index)
  {
    assert(index < _ovars.size());
    return _ovars[index];
  }
