...
typedef tools::Real nsignal;
typedef tools::Real nweight;
...
class afunction
{
 public:
  ...
  virtual afunction* getDerivative()
  {
    return NULL;
  }
  virtual afunction* getPrimitive()
  {
    return NULL;
  }
  ...
  nsignal calcIntegral(const nsignal& a, const nsignal& b,
                       const nsignal& delta=_DELTAI)
  {
    nsignal sum = 0.0;
    nsignal mult = 1.0;
...
{
 public:
  pfunction(afunction* a)
  {
    _func = a;
    _isDerivable = a->isDerivable();
  }
  ~pfunction() {}

  nsignal calculate(const nsignal& x)
  {
    return (_func)? _func->calculate(x) : 0.0f;
  }
...
{
 public:
  addFunc(afunction* f, afunction* g)
  {
    _f = f;
    _g = g;
  ...
  {
    return  (_isDerivable)? new addFunc(_f->getDerivative(),
                                        _g->getDerivative()) : NULL;
  }
...
{
 public:
  multiplyFunc(afunction* f, afunction* g)
  {
    _f = f;
    _g = g;
    _isDerivable = (_f && _g)? (_f->isDerivable() && _g->isDerivable()) : false;
  ...
  afunction* getDerivative()
  {
    return  (_isDerivable)? new addFunc(new multiplyFunc(new pfunction(_f),
                                        _g->getDerivative()), \
...
{
 public:
  compositeFunc(afunction* f, afunction* g)
  {
    _f = f;
    _g = g;
    _isDerivable = (_f && _g)? (_f->isDerivable() && _g->isDerivable()) : false;
  ...
  afunction* getDerivative()
  {
    return (_isDerivable)? new multiplyFunc(_f->getDerivative(),
                                            _g->getDerivative()) : NULL;
  }
...
{
 public:
  stepFunction(const nsignal& low=0, const nsignal& high=1)
  {
    _low = low;
    _high = high;
  }
...
class constFunction : public afunction
{
 public:
  ...
  nsignal calculate(const nsignal& x=0.0f)
  {
    return _k;
  }
  afunction* getDerivative()
  {
    return new constFunction(0.0f);
  }
...
class idFunction : public afunction
{
 public:
  ...
  nsignal calculate(const nsignal& ninput)
  {
    return ninput;
  }
  afunction* getDerivative()
  {
    return new constFunction(1.0f);
  }
};
...
{
 public:
  gaussFunction(const double& avg, const double& stdev)
  {
    setParameters(avg, stdev);
  }
  ~gaussFunction() {}

  nsignal calculate(const nsignal& x)
  {
    return _a * exp( -pow(x - _b, 2) / (2 * pow(_c, 2)) );
  }
...
class tanhFunction : public afunction
{
 public:
  ...
  afunction* getDerivative()
  {
    return new compositeFunc(new squareFunction(), \
                             new sechFunction());
  }
};
