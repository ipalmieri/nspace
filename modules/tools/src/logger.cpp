...
string tools::formatLong(const string &msg, const string &sname, const logLevel &level)
{
   //time_t t = time(NULL);  
   ...
   string lstr = logger::levelToStr(level);
...
string tools::formatShort(const string &msg, const string &sname, const logLevel &level)
{
   string lname = (sname.size() > 0 && sname != _LOGGER_DSNAME)? "[" + sname + "] " :  "";

   return lname + msg;
}


// === logObject derived classes ===

void logFile::printMsg(const string &msg, const string &sname, const logLevel &level)
{
   
   string message = formatLong(msg, sname, level);
...
void logSyslog::printMsg(const string &msg, const string &sname, const logLevel &level)
{

   string message = formatShort(msg, sname, level);
...
int logSyslog::levelToSyslog(const logLevel &level)
{
   ...
   switch (level)
   {
      case LGR_DEBUG: return LOG_DEBUG;
      case LGR_INFO: return LOG_INFO;
      case LGR_WARNING: return LOG_WARNING;
      case LGR_ERROR: return LOG_ERR;
      default: break;
   }
...
void logTerminal::printMsg(const string &msg, const string &sname, const logLevel &level)
{
   string message = formatLong(msg, sname, level);
...
logSource::~logSource()
{
   for (vector<logObject *>::iterator it = objects.begin(); it != objects.end(); ++it)
   {
      delete *it;
   }
}
...
void logger::logServerMsg(const string &msg, const string &sname, const logLevel &level)
{
   logSource *nsrc  = getSource(sname);
...
void logger::startLog(const string &filename, const string &sname, const logLevel &level)
{
...
void logger::logMsg(const string &msg, const string &sname, const logLevel &level)
{
   logger *ls = logServer::getInstance();
...
// print log level string from enum
string logger::levelToStr(const logLevel &level)
{
   switch (level)
   {
      case LGR_DEBUG: return string("DEBUG");
      case LGR_INFO: return string("INFO");
      case LGR_WARNING: return string("WARNING");
      case LGR_ERROR: return string("ERROR");
      default: break;
   }
