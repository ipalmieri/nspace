...
imageMap::imageMap(const unsigned& width, const unsigned& height,
...
void imageMap::flipVertical()
{
  ...
  if (!temp) {
    throw runtime_error("Can't malloc() from imageMap.flipVertical.");
  }
...
void imageMap::reset(const unsigned& width, const unsigned& height,
                     const unsigned& channels)
{
