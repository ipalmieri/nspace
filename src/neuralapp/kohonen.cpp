...
//fix this
vector<nsignal> kohonenNet::spatialUMatrix(const double &radius)
{
   vector<nsignal> ret;

   for(vector<neuron *>::iterator it = _ovars.begin(); it != _ovars.end(); ++it)
   {
      ret.push_back(avgDist((kneuron *)(*it), radius));
   }
...
kTrainer::kTrainer(kohonenNet *kmap, const double &psigma, const double &plambda)
{
   _kmap = kmap;
   _sigma0 = psigma;
   _lambda = plambda;
...
void kTrainer::updateWeights(const double &px, const double &py, const double &pz)
{
   ...
   for(vector<neuron *>::iterator it = _kmap->_ovars.begin(); it != _kmap->_ovars.end(); ++it)
   {
      ...
      for(edgeMap::const_iterator eit = nd->inputEdgesBegin(); eit != nd->inputEdgesEnd(); ++eit)
      {
...
{
   vector<nsignal> outp;
   outp = _kmap->spatialUMatrix(radius);
...
void kMan::saveImage(const string &filename, vector<nsignal> outp)
{
