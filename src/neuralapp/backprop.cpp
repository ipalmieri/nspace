...
void backNet::cloneNet(neuralNet* net)
{
  ...
  //creates backNeurons
  for (nodeMap::const_iterator it = graph->nodeMapBegin();
       it != graph->nodeMapEnd(); ++it) {
  ...
  for (edgeMap::const_iterator it = graph->edgeMapBegin();
       it != graph->edgeMapEnd(); ++it) {
...
backProp::backProp(neuralNet* net, const nsignal& neta)
{
  ...
  _forwardNet = net;
  _forwardNet->initWeights(-1.0f, 1.0f);
  _forwardNet->initBias(-1.0f, 1.0f);
...
void backProp::addData(vectorN<nsignal>& input, vectorN<nsignal>& output)
{
  assert(input.size() == _forwardNet->inputSize() &&
         output.size() == _forwardNet->outputSize());

  if (input.size() == _forwardNet->inputSize() &&
      output.size() == _forwardNet->outputSize()) {
...
void backProp::stepEpoch()
{
  ...
  for (nodeMap::const_iterator it = gp->nodeMapBegin(); it != gp->nodeMapEnd();
       ++it) {
  ...
  for (edgeMap::const_iterator it = gp->edgeMapBegin(); it != gp->edgeMapEnd();
       ++it) {
...
Real backProp::netError()
{
  ...
  for (unsigned i = 0; i < _inData.size(); i++) {
    ...
    vectorN<nsignal> outp = _forwardNet->getOutput();
