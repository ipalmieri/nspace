...
//=================== gaNetTrainer ========================
gaNetTrainer::gaNetTrainer(neuralNet *net, const unsigned &pinit) : gaMethod()
{
   _net = net;
...
void gaNetTrainer::addData(vector<nsignal> &input, vector<nsignal> &output)
{
...
void gaNetTrainer::init(const unsigned &pinit)
{
   ...
   for (unsigned i = 0; i < pinit; i++)
   {
	 ...
	 index++;
      ...
      for (nodeMap::const_iterator it = gr->nodeMapBegin(); it != gr->nodeMapEnd(); ++it)
      {
...
nsignal gaNetTrainer::evalError(chromossome *chr)
{
   ...
   for (unsigned i = 0; i < _inData.size(); i++)
   {
	 ...
	 erro += (double)realAbs(outputError(_ouData[i]));
	 mc++;
      }/*
      else
	 cout << "ERRO!! ";
...
Real gaNetTrainer::outputError(vector<nsignal> &ynet)
{
   Real err = 0.0f;
   vector<nsignal> outp = _net->getOutput();
